...
// -------------------------------------------------
// ✅ เพิ่มปุ่ม “เพิ่มรายการ” (Add Item Modal)
// ✅ รองรับฐานข้อมูลออนไลน์ผ่าน Supabase (อ่าน/เขียนแบบแชร์กัน, realtime sync)
// ✅ ถ้าไม่ได้ตั้งค่า Supabase จะ fallback เป็น localStorage เหมือนเดิม
// ⚠️ หมายเหตุ: ติดตั้งไลบรารีก่อนใช้งาน `npm i @supabase/supabase-js`
// ตั้งค่า ENV (Vite): VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY
...
function filterAndSort(
  items: Item[],
  filters: { q?: string; category?: string; status?: Status | "ทั้งหมด"; location?: string; onlyLow?: boolean; },
...
export default function InventoryApp() {
  const [items, setItems] = useState<Item[]>(() => {
    ...
    return []; // จะโหลดจาก Supabase ใน useEffect
  });

  // editable checker list (persisted locally — สามารถย้ายขึ้นฐานข้อมูลภายหลังได้)
  ...
  async function confirmQuickCheck() {
    ...
    if (hasSupabase && supabase) {
      await supabase.from("items").update({ checked_by: list, last_updated: nowISO() }).eq("id", checkTarget.id);
    }
  ...
  const [form, setForm] = useState({
    ...
    qty: 1,
    unit: "ชิ้น",
    location: "ชั้นเก็บของ",
  });
  function resetForm() {
    setForm({ category: "", name: "", qty: 1, unit: "ชิ้น", location: "ชั้นเก็บของ" });
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error) {
        ...
        // fallback local
        const raw = localStorage.getItem(STORAGE_KEY);
        setItems(raw ? (JSON.parse(raw) as Item[]) : SEED);
      } else {
    ...
    const channel = supabase.channel("public:items")
      .on(
        ...
        (payload: RealtimePostgresChangesPayload<any>) => {
          setItems(prev => mergeUpsert(prev, rowToItem(payload.new)));
        }
      )
      .on(
        "postgres_changes",
  ...
  function rowToItem(r: any): Item {
    return {
      ...
      qty: Number(r.qty ?? 0),
      unit: r.unit,
      status: (r.status as Status) ?? "ปกติ",
      location: r.location,
  ...
  function mergeUpsert(prev: Item[], incoming: Item) {
    const exists = prev.some(x => x.id === incoming.id);
    return exists ? prev.map(x => x.id === incoming.id ? incoming : x) : [...prev, incoming].sort((a,b)=>a.id-b.id);
  }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800">
      ...
      <header className="sticky top-0 z-20 backdrop-blur bg-white/80 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <button onClick={() => setAddOpen(true)} className="px-3 py-2 rounded-xl border bg-slate-900 text-white hover:bg-slate-800" title="เพิ่มรายการ">+ เพิ่มรายการ</button>
            <button onClick={() => setOnlyLow(v=>!v)} className={classNames("px-3 py-2 rounded-xl border", onlyLow ? "bg-amber-50 border-amber-300 text-amber-800" : "bg-white border-slate-300 hover:bg-slate-50")}>เฉพาะใกล้หมด/หมด</button>
      ...
      {addOpen && (
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <div className="w-full max-w-lg rounded-2xl bg-white shadow-xl border border-slate-200 p-5">
              <h2 className="text-lg font-semibold mb-3">เพิ่มรายการใหม่</h2>
              <div className="grid grid-cols-1 gap-3">
                <TextField label="หมวดหมู่" value={form.category} onChange={(v)=>setForm(s=>({...s, category: v}))} placeholder="เช่น หมวดของใช้ทั่วไป" />
                <TextField label="ชื่อสิ่งของ" value={form.name} onChange={(v)=>setForm(s=>({...s, name: v}))} placeholder="เช่น ถุงมือ size M" />
                <div className="grid grid-cols-3 gap-3">
                  <NumberField label="จำนวน" value={form.qty} min={0} onChange={(v)=>setForm(s=>({...s, qty: v}))} />
                  <TextField label="หน่วย" value={form.unit} onChange={(v)=>setForm(s=>({...s, unit: v}))} placeholder="เช่น กล่อง / แพ็ค / ขวด" />
                  <TextField label="ที่จัดเก็บ" value={form.location} onChange={(v)=>setForm(s=>({...s, location: v}))} placeholder="เช่น ชั้นเก็บของ" />
                </div>
              </div>
              <div className="mt-4 flex justify-end gap-2">
                <button onClick={()=>{ setAddOpen(false); resetForm(); }} className="px-3 py-2 rounded-xl border bg-white">ยกเลิก</button>
                <button onClick={handleAddItem} className="px-3 py-2 rounded-xl bg-slate-900 text-white">บันทึก</button>
              </div>
            </div>
          </div>
        </div>
  ...
  function openEdit(it: Item) {
    // (Editor modal omitted for brevity — สามารถเพิ่มภายหลังได้)
  ...
  async function adjustQty(id: number, delta: number) {
    setItems(prev => prev.map(i => {
      ...
      const updated: Item = { ...i, qty: newQty, status: newStatus, lastUpdated: nowISO() };
      // save online
      if (hasSupabase && supabase) {
        const row = itemToRow(updated);
        supabase.from("items").update(row).eq("id", id).then(({ error })=>{ if (error) console.error(error); });
      }
      return updated;
    }));
  }
  ...
  async function handleAddItem() {
    ...
    if (!name || !category || !unit || !location) { alert("กรอกข้อมูลให้ครบก่อนครับ"); return; }
    const status: Status = autoStatus ? computeStatus(qty, lowThreshold) : "ปกติ";

    if (hasSupabase && supabase) {
      // ให้ฐานข้อมูลสร้าง id เอง (BIGINT GENERATED BY DEFAULT AS IDENTITY)
      const now = nowISO();
      const { data, error } = await supabase.from("items").insert({
        category, name, qty, unit, status, location, checked_by: [], last_updated: now
      }).select().single();
      if (error) { console.error(error); alert("เพิ่มรายการไม่สำเร็จ"); return; }
      const added = rowToItem(data);
      setItems(prev => mergeUpsert(prev, added));
    } else {
      // local mode
      const id = nextId(items);
      const added: Item = { id, category, name, qty, unit, status, location, checkedBy: [], lastUpdated: nowISO() };
      setItems(prev => mergeUpsert(prev, added));
    }
...
function TextField({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; }) {
  return (
    <div>
      <label className="text-sm">{label}</label>
      <input value={value} onChange={(e)=>onChange(e.target.value)} placeholder={placeholder} className="w-full px-3 py-2 rounded-xl border border-slate-300" />
    </div>
  );
}
function NumberField({ label, value, onChange, min = 0 }: { label: string; value: number; onChange: (v: number) => void; min?: number; }) {
  return (
    <div>
      <label className="text-sm">{label}</label>
      <input type="number" value={value} min={min} onChange={(e)=>onChange(Number(e.target.value))} className="w-full px-3 py-2 rounded-xl border border-slate-300" />
    </div>
  );
}
